...
async def refresh_member(session: CommandSession):
    try:
        ...
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令refresh_member时发生错误: {e}, 该命令需要在群组中使用')
    ...
    try:
        for user_info in __group_user_list:
            ...
            if not __is_success:
                log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用refresh_member'
                                   f'更新群组: {session.event.group_id} 时发生错误, '
                                   f"用户: {user_info['user_id']}信息写入数据库失败, 错误信息见日志.")
        await session.send('用户信息已更新~')
        log.logger.info(f'{__name__}: 系统管理员{session.event.user_id} 使用refresh_member'
                        f'更新了群组: {session.event.group_id} 中成员的用户信息')
    except Exception as e:
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令refresh_member时发生错误: {e}, 更新用户信息失败')
...
async def refresh_group(session: CommandSession):
    try:
        ...
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令refresh_group时发生错误: {e}, 该命令需要在群组中使用')
        session.finish('请在群组内使用该命令！')
        return
    try:
        ...
        if not __is_success:
            ...
            log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用refresh_group'
                               f'更新群组: {session.event.group_id} 时发生错误, 数据库操作失败, 错误信息见日志.')
            return
        await session.send('群组信息已更新~')
        log.logger.info(f'{__name__}: 系统管理员{session.event.user_id} 使用refresh_group'
                        f'更新了群组: {session.event.group_id} 的群组信息')
    except Exception as e:
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令refresh_group时发生错误: {e}, 更新群组信息失败')
...
async def refresh_member_group(session: CommandSession):
    try:
        ...
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令refresh_member_group时发生错误: {e}, 该命令需要在群组中使用')
        session.finish('请在群组内使用该命令！')
        return
    try:
        for user_info in __group_user_list:
            ...
            if not __is_success:
                log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用refresh_member_group'
                                   f'更新群组: {session.event.group_id} 时发生错误, '
                                   f"用户: {__user_qq}信息写入数据库失败, 错误信息见日志.")
        await session.send('群组群组成员列表已更新~')
        log.logger.info(f'{__name__}: 系统管理员{session.event.user_id} 使用refresh_member_group'
                        f'更新了群组: {session.event.group_id} 的群组成员列表')
    except Exception as e:
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令refresh_member_group时发生错误: {e}, 更新群组成员列表失败')
...
async def reset_member_status(session: CommandSession):
    try:
        ...
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令reset_member_status时发生错误: {e}, 该命令需要在群组中使用')
        session.finish('请在群组内使用该命令！')
        return
    try:
        for user_info in __group_user_list:
        ...
        log.logger.info(f'{__name__}: 系统管理员{session.event.user_id} 使用reset_member_status'
                        f'重置了群组: {session.event.group_id} 的群组成员状态')
    except Exception as e:
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令reset_member_status时发生错误: {e}, 重置成员状态失败')
...
async def query_group_permissions(session: CommandSession):
    try:
        ...
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} '
    ...
    try:
        ...
        log.logger.info(f'{__name__}: 系统管理员{session.event.user_id} 使用query_group_permissions'
                        f'查询了群组: {session.event.group_id} 的群组权限')
    except Exception as e:
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令query_group_permissions时发生错误: {e}, 查询群组权限失败')
...
async def reset_group_permissions(session: CommandSession):
    ...
    try:
        ...
        if not __is_success:
            await session.send('发生了意料之外的错误OvO')
            log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用reset_group_permissions'
                               f'重置群组: {session.event.group_id} 时发生错误, 数据库操作失败, 错误信息见日志.')
            return
        await session.send('本群组权限已重置~')
        log.logger.info(f'{__name__}: 系统管理员{session.event.user_id} 使用reset_group_permissions'
                        f'重置了群组: {session.event.group_id} 的群组权限')
    except Exception as e:
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令reset_group_permissions时发生错误: {e}, 重置群组权限失败')
...
async def set_group_permissions(session: CommandSession):
    try:
        ...
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令set_group_permissions时发生错误: {e}, 该命令需要在群组中使用')
        session.finish('请在群组内使用该命令！')
        return
    ...
    try:
        ...
        if not __is_success:
            await session.send('发生了意料之外的错误OvO')
            log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用set_group_permissions'
                               f'更新群组: {session.event.group_id} 时发生错误, 数据库操作失败, 错误信息见日志.')
            return
        await session.send('已成功配置本群组权限~')
        log.logger.info(f'{__name__}: 系统管理员{session.event.user_id} 使用set_group_permissions'
                        f'配置了群组: {session.event.group_id} 的群组权限')
    except Exception as e:
        log.logger.warning(f'{__name__}: 系统管理员{session.event.user_id} 使用命令set_group_permissions时发生错误: {e}, 设置群组权限失败')
